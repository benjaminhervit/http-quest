...
def createTheGitLevel():
    ...
    level.set_riddle(f'If you can remember the right order of the paralysis spell, the you can PUT {R.STUN_THE_GIT_MONSTER.value} the git monster to sleep - at least for a while. How was it? pull commit add push? or... push commit pull add? or was it force rebase merge origin? If you do it right, you have a 20% URL chance to succeed')
...
def createRegisterLevel():
    ...
    the_register_level.set_riddle('You must register your party to take on this CRUDe quest.')
    ...
    the_register_level.set_victory_message_template("""Thank you for taking on this CRUDe quest!
                                               Wauw... {party} is such as cool name... ... Anyway!
                                               This is JaSON. You should take him with you as your translator.
                                               Also, consider bring a POSTMAN along as well. They can be very APPlicable in more situations than one might think. Best of luck!
                                               """)
...
def createTheTestLevel():
    ...
    the_test_level.set_riddle('To know that you are worthy, you must answer: Answer to the Ultimate Question of Life, The Universe, and Everything?')
    the_test_level.set_hint(f"""
                       Grab your towel and hitch a hike around the internet and you will find what you are seeking.
                       When you have an answer, POST it to {R.THE_TEST_ANSWER.value}.
                       """)
...
def createGateLevel():
    ...
    level.set_riddle(f'To open {R.THE_GATE.value} you must DELETE it - but you have not taken your pre-workout shake today. Instead, you must [answer] an riddle in old forgotten language: "Speak friend and enter..."')
...
def createThroneLevel():
    ...
    level.set_riddle(f'Standing in fron of the throne you see the CRUDe Crown and you know what you must do. But how?')
    level.set_hint(f'JaSON seems to know the answer but can you ask him the right question to PUT on {R.THE_CROWN.value}?? You have a feeling that it is question you tried to answer yourself in the very beginning of thsi quest.')
    level.set_wrong_answer_response("JaSON smiles but remains silent. This is not it. Maybe you should just hitchhike out of here?")
    level.set_victory_message_template(f'HOW THE FUCK SHOULD I KNOW SOMETHING LIKE THAT?! JaSON exlaims when he hears your question.')
    return level

def createCrownLevel():
    level = CrownLevel()
    last_instructions = f'JUST... Request with method PUT to {R.THE_CROWN.value} and body as JSON: {{"party":"name", "answer":"i have a cool new hat"}}. How hard can it be?! And my name is Terry! JaSoOoNn is just a part that I am playing!'
    level.set_directions(last_instructions)
    level.set_riddle('In case JaSONs immature outburst made you forget the setting: Standing in fron of the throne you see the CRUDe Crown and you know what you must do. But how?')
    level.set_hint(last_instructions)
    level.set_wrong_answer_response("JaSON look at you with a thousand yeard stare and a frozen smile... I think we broke JaSON? Wait! It seems like he is preparing to say something? : ")
    level.set_victory_message_template('You PUT on the CRUDe CROWN victoriously ignoreing JaSONs sulky attitude. You did it! You are now the Ruler of WoT with a cool hat. Well done!')
    return level