from level import Level, LevelE as L
import level_builder
from routes import Route as R
from status_code import StatusCode

import request_validator as rv
...
def execute_level(level:Level, next_level:Level, target_method, method, party_data_found, party, answer, request_is_answer):
    """
    ...
    """
    method_response = rv.create_method_response(method, target_method)
    if not method_response['success']:
        return jsonify(method_response)
    
    data_response = rv.create_data_response(party_data_found, party, get_db())
    if not data_response['success']:
        return jsonify(data_response)
    
    if request_is_answer:
        answer_response = rv.create_answer_response(level, next_level, answer, party)
        if answer_response['success']:
            get_db().update_score(party, 1)
        return jsonify(answer_response)
    return jsonify(rv.create_level_welcome_response(level))
...
def register():
    ...
    data_response = rv.create_data_response(party_in_form, party, get_db())
    if data_response['success']:
        data_response.update({'message':'Seems like this party is already registered?'})
        return jsonify(data_response)
    
    method_response = rv.create_method_response(request.method, 'POST')
    if not method_response['success']:
        return jsonify(method_response)
    ...
    success_response = rv.create_answer_response(level, next_level, True, data_response['party'])
