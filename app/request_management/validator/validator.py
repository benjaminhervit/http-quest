...
class Validator:
    ...
    def validate(self, parsed: dict):
        ...
        content_is_valid = self._validate_content(content=content)
        
        username = parsed.get(ParserKey.USERNAME.value)
        username_is_valid = self._validate_username(username=username)
        
        token = parsed.get(ParserKey.TOKEN.value)
        token_is_valid = self._validate_token(token=token)
        
        return (method_is_valid and content_is_valid
                and username_is_valid and token_is_valid)
