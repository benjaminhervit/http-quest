...
def no_data_expected(*args, **kwargs):
    ...
    if has_data:
        raise ValidationError('No data expected but found content', code=StatusCode.BAD_REQUEST)
    return True
...
def any_data_expected(*args, **kwargs):
    ...
    if no_data:
        raise ValidationError('Expected any data but received nothing', code=StatusCode.BAD_REQUEST)
    return True