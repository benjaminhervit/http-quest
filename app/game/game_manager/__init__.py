...
from app.enums import QuestState, StatusCode, ParsingKey, PlayerAction
...
def execute_quest(q:QuestData, p:ParsedRequest):
    ...
    username = p.get(ParsingKey.USERNAME.value)
    answer = p.get(ParsingKey.ANSWER.value)
...
def _get_action(p:ParsedRequest) -> PlayerAction:
    try:
        return PlayerAction(p.get(ParsingKey.ACTION_TYPE))
